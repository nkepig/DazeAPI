...
const MessageContent = ({
  ...
  if (message.status === 'error') {
    ...
    if (Array.isArray(message.content)) {
      ...
      errorText = textContent && textContent.text && typeof textContent.text === 'string' ? textContent.text : "请求发生错误";
  ...
  if (message.role === 'assistant' && isThinkingStatus && !finalExtractedThinkingContent && (!finalDisplayableFinalContent || finalDisplayableFinalContent.trim() === '')) {
    return <div className={`${className} flex items-center gap-2 sm:gap-4 bg-gradient-to-r from-purple-50 to-indigo-50`}>
  ...
  return <div className={className}>
      {message.role === 'system' && <div className='mb-2 sm:mb-4'>
          <div className='flex items-center gap-2 p-2 sm:p-3 bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg' style={{
      ...
      {message.role === 'assistant' && <ThinkingContent message={message} finalExtractedThinkingContent={finalExtractedThinkingContent} thinkingSource={thinkingSource} styleState={styleState} onToggleReasoningExpansion={onToggleReasoningExpansion} />}

      {isEditing ? <div className='space-y-3'>
          <TextArea value={editValue} onChange={value => onEditValueChange(value)} placeholder={"请输入消息内容..."} autosize={{
        minRows: 3,
        maxRows: 12
      }} style={{
        resize: 'vertical',
        ...
        lineHeight: '1.6'
          ...
          <div className='flex items-center gap-2 w-full'>
            <Button size='small' type='danger' theme='light' icon={<X size={14} />} onClick={onEditCancel} className='flex-1'>

              {"取消"}
            </Button>
            <Button size='small' type='warning' theme='solid' icon={<Check size={14} />} onClick={onEditSave} disabled={!editValue || editValue.trim() === ''} className='flex-1'>
      ...
      if (Array.isArray(message.content)) {
        ...
        return <div>
                {imageContents.length > 0 && <div className='mb-3 space-y-2'>
                    {imageContents.map((imgItem, index) => <div key={index} className='max-w-sm'>
                        <img src={imgItem.image_url.url} alt={`用户上传的图片 ${index + 1}`} className='rounded-lg max-w-full h-auto shadow-sm border' style={{
                maxHeight: '300px'
              }} onError={e => {
                ...
                e.target.nextSibling.style.display = 'block';
                        ...
                        <div className='text-red-500 text-sm p-2 bg-red-50 rounded-lg border border-red-200' style={{
                display: 'none'
              }}>{"\n                          图片加载失败: "}
                ...
                {textContent && textContent.text && typeof textContent.text === 'string' && textContent.text.trim() !== '' && <div className={`prose prose-xs sm:prose-sm prose-gray max-w-none overflow-x-auto text-xs sm:text-sm ${message.role === 'user' ? 'user-message' : ''}`}>

                      <MarkdownRenderer content={textContent.text} className={message.role === 'user' ? 'user-message' : ''} animated={false} previousContentLength={0} />
      ...
      if (typeof message.content === 'string') {
        if (message.role === 'assistant') {
          if (finalDisplayableFinalContent && finalDisplayableFinalContent.trim() !== '') {
            ...
            return <div className='prose prose-xs sm:prose-sm prose-gray max-w-none overflow-x-auto text-xs sm:text-sm'>
                    <MarkdownRenderer content={finalDisplayableFinalContent} className='' animated={isThinkingStatus} previousContentLength={prevLength} />

                  </div>;
          }
        } else {
          return <div className={`prose prose-xs sm:prose-sm prose-gray max-w-none overflow-x-auto text-xs sm:text-sm ${message.role === 'user' ? 'user-message' : ''}`}>

                  <MarkdownRenderer content={message.content} className={message.role === 'user' ? 'user-message' : ''} animated={false} previousContentLength={0} />
