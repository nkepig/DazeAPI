...
import { Button, Form, Row, Col, Typography, Modal, Banner, TagInput, Spin, Card, Radio, Select } from '@douyinfe/semi-ui';
const {
  Text
} = Typography;
import { API, removeTrailingSlash, showError, showSuccess, toBoolean } from '../../helpers';
...
const SystemSetting = () => {
  ...
  const updateOptions = async options => {
    ...
    try {
      ...
      const checkboxOptions = options.filter(opt => opt.key.toLowerCase().endsWith('enabled'));
      const otherOptions = options.filter(opt => !opt.key.toLowerCase().endsWith('enabled'));
      ...
      if (otherOptions.length > 0) {
        const requestQueue = otherOptions.map(opt => API.put('/api/option/', {
          key: opt.key,
  ...
  return <div>
      {isLoaded ? <Form initValues={inputs} onValueChange={handleFormChange} getFormApi={api => formApiRef.current = api}>
        ...
        marginTop: '10px'
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Input field='ServerAddress' label={"服务器地址"} placeholder={"https://yourdomain.com"} extraText={"该服务器地址将影响支付回调地址以及默认首页展示的地址，请确保正确配置"} />
              ...
              <Card>
                <Form.Section text={"代理设置"}>
                  <Banner type='info' description={"此代理仅用于图片请求转发，Webhook通知发送等，AI API请求仍然由服务器直接发出，可在渠道设置中单独配置代理"} style={{
              marginBottom: 20,
              marginTop: 16
            }} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field='WorkerUrl' label={"Worker地址"} placeholder={"例如：https://workername.yourdomain.workers.dev"} />

                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field='WorkerValidKey' label={"Worker密钥"} placeholder={"敏感信息不会发送到前端显示"} type='password' />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Checkbox field='fetch_setting.enable_ssrf_protection' noLabel extraText={"SSRF防护开关详细说明"} onChange={e => handleCheckboxChange('fetch_setting.enable_ssrf_protection', e)}>
              ...
              marginTop: 16
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Checkbox field='fetch_setting.allow_private_ip' noLabel extraText={"私有IP访问详细说明"} onChange={e => handleCheckboxChange('fetch_setting.allow_private_ip', e)}>
              ...
              marginTop: 16
                    ...
                    <Col xs={24} sm={24} md={24} lg={24} xl={24}>
                      <Form.Checkbox field='fetch_setting.apply_ip_filter_for_domain' noLabel extraText={"域名IP过滤详细说明"} onChange={e => handleCheckboxChange('fetch_setting.apply_ip_filter_for_domain', e)} style={{
                  marginBottom: 8
                }}>
                  ...
                  marginBottom: 8
                      ...
                      <Radio.Group type='button' value={domainFilterMode ? 'whitelist' : 'blacklist'} onChange={val => {
                  ...
                  marginBottom: 8
                      ...
                      <TagInput value={domainList} onChange={value => {
                  setDomainList(value);
                  // 触发Form的onChange事件
                  setInputs(prev => ({
                    ...prev,
                ...
                }} placeholder={"输入域名后回车，如：example.com"} style={{
                  width: '100%'
                }} />
              ...
              marginTop: 16
                  ...
                  marginBottom: 8
                      ...
                      <Radio.Group type='button' value={ipFilterMode ? 'whitelist' : 'blacklist'} onChange={val => {
                  const selected = val && val.target ? val.target.value : val;
                  const isWhitelist = selected === 'whitelist';
                  ...
                  marginBottom: 8
                      ...
                      <TagInput value={ipList} onChange={value => {
                  setIpList(value);
                  // 触发Form的onChange事件
                  setInputs(prev => ({
                    ...prev,
                ...
                }} placeholder={"输入IP地址后回车，如：8.8.8.8"} style={{
                  width: '100%'
                }} />
              ...
              marginTop: 16
                  ...
                  marginBottom: 8
                      ...
                      <TagInput value={allowedPorts} onChange={value => {
                ...
                }} placeholder={"输入端口后回车，如：80 或 8000-8999"} style={{
                  width: '100%'
                }} />
                  ...
                  <Button onClick={submitSSRF} style={{
              marginTop: 16
            }}>
                    {"更新SSRF防护设置"}
                  </Button>
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Checkbox field='PasswordLoginEnabled' noLabel onChange={e => handleCheckboxChange('PasswordLoginEnabled', e)}>

                        {"允许通过密码进行登录"}
                      </Form.Checkbox>
                      <Form.Checkbox field='PasswordRegisterEnabled' noLabel onChange={e => handleCheckboxChange('PasswordRegisterEnabled', e)}>

                        {"允许通过密码进行注册"}
                      </Form.Checkbox>
                      <Form.Checkbox field='EmailVerificationEnabled' noLabel onChange={e => handleCheckboxChange('EmailVerificationEnabled', e)}>

                        {"通过密码注册时需要进行邮箱验证"}
                      </Form.Checkbox>
                      <Form.Checkbox field='RegisterEnabled' noLabel onChange={e => handleCheckboxChange('RegisterEnabled', e)}>

                        {"允许新用户注册"}
                      </Form.Checkbox>
                      <Form.Checkbox field='TurnstileCheckEnabled' noLabel onChange={e => handleCheckboxChange('TurnstileCheckEnabled', e)}>

                        {"允许 Turnstile 用户校验"}
                      </Form.Checkbox>
                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Checkbox field='GitHubOAuthEnabled' noLabel onChange={e => handleCheckboxChange('GitHubOAuthEnabled', e)}>

                        {"允许通过 GitHub 账户登录 & 注册"}
                      </Form.Checkbox>
                      <Form.Checkbox field='discord.enabled' noLabel onChange={e => handleCheckboxChange('discord.enabled', e)}>

                        {"允许通过 Discord 账户登录 & 注册"}
                      </Form.Checkbox>
                      <Form.Checkbox field='LinuxDOOAuthEnabled' noLabel onChange={e => handleCheckboxChange('LinuxDOOAuthEnabled', e)}>

                        {"允许通过 Linux DO 账户登录 & 注册"}
                      </Form.Checkbox>
                      <Form.Checkbox field='WeChatAuthEnabled' noLabel onChange={e => handleCheckboxChange('WeChatAuthEnabled', e)}>

                        {"允许通过微信登录 & 注册"}
                      </Form.Checkbox>
                      <Form.Checkbox field='TelegramOAuthEnabled' noLabel onChange={e => handleCheckboxChange('TelegramOAuthEnabled', e)}>

                        {"允许通过 Telegram 进行登录"}
                      </Form.Checkbox>
                      <Form.Checkbox field="['oidc.enabled']" noLabel onChange={e => handleCheckboxChange('oidc.enabled', e)}>
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Checkbox field='EmailDomainRestrictionEnabled' noLabel onChange={e => handleCheckboxChange('EmailDomainRestrictionEnabled', e)}>{"\n                        启用邮箱域名白名单\n                      "}


                  </Form.Checkbox>
                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Checkbox field='EmailAliasRestrictionEnabled' noLabel onChange={e => handleCheckboxChange('EmailAliasRestrictionEnabled', e)}>{"\n                        启用邮箱别名限制\n                      "}


                  </Form.Checkbox>
                    </Col>
                  </Row>
                  <TagInput value={emailDomainWhitelist} onChange={setEmailDomainWhitelist} placeholder={"输入域名后回车"} style={{
              width: '100%',
              marginTop: 16
            }} />

                  <Form.Input placeholder={"输入要添加的邮箱域名"} value={emailToAdd} onChange={value => setEmailToAdd(value)} style={{
              marginTop: 16
            }} suffix={<Button theme='solid' type='primary' onClick={handleAddEmail}>

                        {"添加"}
                      </Button>} onEnterPress={handleAddEmail} />

                  <Button onClick={submitEmailDomainWhitelist} style={{
              marginTop: 10
            }}>
              ...
              marginTop: 16
                    ...
                    <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                      <Form.Input field='SMTPToken' label={"SMTP 访问凭证"} type='password' placeholder={"敏感信息不会发送到前端显示"} />

                    </Col>
                    <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                      <Form.Checkbox field='SMTPSSLEnabled' noLabel onChange={e => handleCheckboxChange('SMTPSSLEnabled', e)}>
              ...
              <Card>
                <Form.Section text={"配置 OIDC"}>
                  ...
                  <Banner type='info' description={`${"主页链接填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}，${"重定向 URL 填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}/oauth/oidc`} style={{
              marginBottom: 20,
              marginTop: 16
            }} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['oidc.well_known']" label={"Well-Known URL"} placeholder={"请输入 OIDC 的 Well-Known URL"} />

                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['oidc.client_id']" label={"Client ID"} placeholder={"输入 OIDC 的 Client ID"} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['oidc.client_secret']" label={"Client Secret"} type='password' placeholder={"敏感信息不会发送到前端显示"} />

                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['oidc.authorization_endpoint']" label={"Authorization Endpoint"} placeholder={"输入 OIDC 的 Authorization Endpoint"} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['oidc.token_endpoint']" label={"Token Endpoint"} placeholder={"输入 OIDC 的 Token Endpoint"} />

                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['oidc.user_info_endpoint']" label={"User Info Endpoint"} placeholder={"输入 OIDC 的 Userinfo Endpoint"} />
              ...
              <Card>
                <Form.Section text={"配置 GitHub OAuth App"}>
                  ...
                  <Banner type='info' description={`${"Homepage URL 填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}，${"Authorization callback URL 填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}/oauth/github`} style={{
              marginBottom: 20,
              marginTop: 16
            }} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field='GitHubClientSecret' label={"GitHub Client Secret"} type='password' placeholder={"敏感信息不会发送到前端显示"} />
              ...
              <Card>
                <Form.Section text={"配置 Discord OAuth"}>
                  ...
                  <Banner type='info' description={`${"Homepage URL 填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}，${"Authorization callback URL 填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}/oauth/discord`} style={{
              marginBottom: 20,
              marginTop: 16
            }} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['discord.client_id']" label={"Discord Client ID"} />

                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field="['discord.client_secret']" label={"Discord Client Secret"} type='password' placeholder={"敏感信息不会发送到前端显示"} />
              ...
              <Card>
                <Form.Section text={"配置 Linux DO OAuth"}>
                  <Text>
                    ...
                    <a href='https://connect.linux.do/' target='_blank' rel='noreferrer' style={{
                ...
                marginRight: 4
                  ...
                  <Banner type='info' description={`${"回调 URL 填"} ${inputs.ServerAddress ? inputs.ServerAddress : "网站地址"}/oauth/linuxdo`} style={{
              marginBottom: 20,
              marginTop: 16
            }} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={10} lg={10} xl={10}>
                      <Form.Input field='LinuxDOClientId' label={"Linux DO Client ID"} placeholder={"输入你注册的 LinuxDO OAuth APP 的 ID"} />

                    </Col>
                    <Col xs={24} sm={24} md={10} lg={10} xl={10}>
                      <Form.Input field='LinuxDOClientSecret' label={"Linux DO Client Secret"} type='password' placeholder={"敏感信息不会发送到前端显示"} />

                    </Col>
                    <Col xs={24} sm={24} md={4} lg={4} xl={4}>
                      <Form.Input field='LinuxDOMinimumTrustLevel' label={"LinuxDO Minimum Trust Level"} placeholder={"允许注册的最低信任等级"} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                      <Form.Input field='WeChatServerAddress' label={"WeChat Server 服务器地址"} />

                    </Col>
                    <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                      <Form.Input field='WeChatServerToken' label={"WeChat Server 访问凭证"} type='password' placeholder={"敏感信息不会发送到前端显示"} />

                    </Col>
                    <Col xs={24} sm={24} md={8} lg={8} xl={8}>
                      <Form.Input field='WeChatAccountQRCodeImageURL' label={"微信公众号二维码图片链接"} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field='TelegramBotToken' label={"Telegram Bot Token"} placeholder={"敏感信息不会发送到前端显示"} type='password' />

                    </Col>
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field='TelegramBotName' label={"Telegram Bot 名称"} />
              ...
              xxl: 24
                    ...
                    <Col xs={24} sm={24} md={12} lg={12} xl={12}>
                      <Form.Input field='TurnstileSecretKey' label={"Turnstile Secret Key"} type='password' placeholder={"敏感信息不会发送到前端显示"} />
              ...
              <Modal title={"确认取消密码登录"} visible={showPasswordLoginConfirmModal} onOk={handlePasswordLoginConfirm} onCancel={() => {
          setShowPasswordLoginConfirmModal(false);
