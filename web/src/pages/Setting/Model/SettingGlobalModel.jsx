...
import { Button, Col, Form, Row, Spin, Banner, Tag, Divider } from '@douyinfe/semi-ui';
import { compareObjects, API, showError, showSuccess, showWarning, verifyJSON } from '../../../helpers';
const thinkingExample = JSON.stringify(['moonshotai/kimi-k2-thinking', 'kimi-k2-thinking'], null, 2);
...
export default function SettingGlobalModel(props) {
  ...
  const chatCompletionsToResponsesPolicyKey = 'global.chat_completions_to_responses_policy';
  ...
  function onSubmit() {
    ...
    const requestQueue = updateArray.map(item => {
      const normalizedValue = normalizeValueBeforeSave(item.key, inputs[item.key]);
  ...
  useEffect(() => {
    ...
    for (const key of Object.keys(defaultGlobalSettingInputs)) {
      if (props.options[key] !== undefined) {
        ...
        if (key === 'global.thinking_model_blacklist') {
          try {
            value = value && String(value).trim() !== '' ? JSON.stringify(JSON.parse(value), null, 2) : defaultGlobalSettingInputs[key];
          } catch (error) {
        ...
        marginBottom: 15
          ...
          <Form.Section text={"全局设置"}>
            ...
            <Row>
              ...
              }]} extraText={"列出的模型将不会自动添加或移除-thinking/-nothinking 后缀"} onChange={value => setInputs({
                ...inputs,
            ...
            flexWrap: 'wrap'
              ...
              marginTop: 10
                ...
                <Col span={24}>
                  <Banner type='warning' description={"提示：该功能为测试版，未来配置结构与功能行为可能发生变更，请勿在生产环境使用。"} />
              ...
              marginTop: 10
                ...
                <Col span={24}>
                  <Form.TextArea label={"参数配置"} field={chatCompletionsToResponsesPolicyKey} placeholder={"例如（指定渠道）：" + '\n' + chatCompletionsToResponsesPolicyExample + '\n\n' + "例如（全渠道）：" + '\n' + chatCompletionsToResponsesPolicyAllChannelsExample} rows={8} rules={[{
                  validator: (rule, value) => {
                    if (!value || value.trim() === '') return true;
                    return verifyJSON(value);
                  },
              ...
              marginBottom: 16
                ...
                <Col span={24}>
                  ...
                  alignItems: 'center'
                    ...
                    <Button type='secondary' size='small' onClick={() => setChatCompletionsToResponsesPolicyValue(chatCompletionsToResponsesPolicyExample)}>

                      {"填充模板（指定渠道）"}
                    </Button>
                    <Button type='secondary' size='small' onClick={() => setChatCompletionsToResponsesPolicyValue(chatCompletionsToResponsesPolicyAllChannelsExample)}>
            ...
            fontWeight: 600
              ...
              marginTop: 10
                ...
                <Col span={24}>
                  <Banner type='warning' description={"警告：启用保活后，如果已经写入保活数据后渠道出错，系统无法重试，如果必须开启，推荐设置尽可能大的Ping间隔"} />

                </Col>
              </Row>
              <Row>
                <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                  <Form.Switch label={"启用Ping间隔"} field={'general_setting.ping_interval_enabled'} onChange={value => setInputs({
                  ...inputs,
                ...
                <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                  <Form.InputNumber label={"Ping间隔（秒）"} field={'general_setting.ping_interval_seconds'} onChange={value => setInputs({
                  ...inputs,
                  'general_setting.ping_interval_seconds': value
                })} min={1} disabled={!inputs['general_setting.ping_interval_enabled']} />
