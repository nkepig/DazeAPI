...
import { Banner, Button, Col, Collapse, Divider, Form, Input, Modal, Row, Select, Space, Spin, Table, Tag, Typography } from '@douyinfe/semi-ui';
import { IconClose, IconCode, IconDelete, IconEdit, IconPlus, IconRefresh, IconSearch } from '@douyinfe/semi-icons';
import { API, compareObjects, showError, showSuccess, showWarning, toBoolean, verifyJSON } from '../../../helpers';
import { CHANNEL_AFFINITY_RULE_TEMPLATES, cloneChannelAffinityTemplate } from '../../../constants/channel-affinity-template.constants';
...
const CONTEXT_KEY_PRESETS = [{
  ...
  key: 'specific_channel_id',
  label: 'specific_channel_id'
}];
...
const tryParseRulesJsonArray = jsonString => {
  ...
  if (!verifyJSON(raw)) return {
    ok: false,
    message: 'Rules JSON is invalid'
  };
  try {
...
export default function SettingsChannelAffinity(props) {
  ...
  const [paramTemplateEditorVisible, setParamTemplateEditorVisible] = useState(false);
  const effectiveDefaultTTLSeconds = Number(inputs?.[KEY_DEFAULT_TTL] || 0) > 0 ? Number(inputs?.[KEY_DEFAULT_TTL] || 0) : 3600;
  const buildModalFormValues = rule => {
    ...
    return {
      ...
      param_override_template_json: r.param_override_template ? stringifyPretty(r.param_override_template) : ''
    };
  };
  ...
  const confirmClearRuleCache = rule => {
    ...
    if (!rule?.include_rule_name) {
      showWarning("该规则未启用“作用域：包含规则名称”，无法按规则清空缓存。");
      return;
    }
    Modal.confirm({
      title: "确认清空该规则缓存",
  ...
  const appendCodexAndClaudeCodeTemplates = () => {
    const doAppend = () => {
      const existingNames = new Set((rules || []).map(r => (r?.name || '').trim()).filter(x => x.length > 0));
      const templates = [CHANNEL_AFFINITY_RULE_TEMPLATES.codexCli, CHANNEL_AFFINITY_RULE_TEMPLATES.claudeCli].map(tpl => {
  ...
  const ruleColumns = [{
    ...
    render: list => (list || []).length > 0 ? (list || []).slice(0, 3).map((v, idx) => <Tag key={`${v}-${idx}`} style={{
      marginRight: 4
    }}>
                {v}
              </Tag>) : '-'
    ...
    render: list => (list || []).length > 0 ? (list || []).slice(0, 2).map((v, idx) => <Tag key={`${v}-${idx}`} style={{
      marginRight: 4
    }}>
                {v}
              </Tag>) : '-'
    ...
    render: list => {
      ...
      return xs.slice(0, 3).map((src, idx) => {
        ...
        return <Tag key={`${s.type}-${idx}`} style={{
          marginRight: 4
        }}>
              {s.type}:{detail}
            </Tag>;
    ...
    render: value => <Tag color={value ? 'orange' : 'grey'} style={{
      marginRight: 4
    }}>
          {value ? "是" : "否"}
        </Tag>
    ...
    render: (_, record) => {
      ...
      return <Button size='small' icon={<IconSearch />} type='tertiary' onClick={() => openParamTemplatePreview(record)}>
    ...
    render: (_, record) => {
      ...
      return tags.map(x => <Tag key={x} style={{
        marginRight: 4
      }}>
            {x}
          </Tag>);
    ...
    render: (_, record) => <Space>
          <Button icon={<IconClose />} theme='borderless' type='warning' disabled={!record?.include_rule_name} title={"清空该规则缓存"} aria-label={"清空该规则缓存"} onClick={() => confirmClearRuleCache(record)} />

          <Button icon={<IconEdit />} theme='borderless' title={"编辑规则"} aria-label={"编辑规则"} onClick={() => handleEditRule(record)} />

          <Button icon={<IconDelete />} theme='borderless' type='danger' title={"删除规则"} aria-label={"删除规则"} onClick={() => handleDeleteRule(record.id)} />
  ...
  const handleDeleteRule = id => {
    ...
    updateRulesState(next.map((r, idx) => ({
      ...r,
      id: idx
    })));
    showSuccess("删除成功");
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (idx < 0 && editingRule?.name) {
          idx = next.findIndex(r => (r?.name || '').trim() === (editingRule?.name || '').trim());
        }
  ...
  async function onSubmit() {
    ...
    if (!verifyJSON(inputs[KEY_RULES] || '[]')) return showError("规则 JSON 格式不正确");
    let compactRules;
    try {
    ...
    Promise.all(requestQueue).then(res => {
      ...
      showSuccess("保存成功");
      props.refresh();
    }).catch(() => showError("保存失败，请重试")).finally(() => setLoading(false));
  }
  useEffect(() => {
    const currentInputs = {
      ...inputs
    ...
    for (let key in props.options) {
      ...
      if (key === KEY_ENABLED) currentInputs[key] = toBoolean(props.options[key]);else if (key === KEY_SWITCH_ON_SUCCESS) currentInputs[key] = toBoolean(props.options[key]);else if (key === KEY_MAX_ENTRIES) currentInputs[key] = Number(props.options[key] || 0) || 0;else if (key === KEY_DEFAULT_TTL) currentInputs[key] = Number(props.options[key] || 0) || 0;else if (key === KEY_RULES) {
        try {
  ...
  const banner = <Banner fullMode={false} type='info' description={"渠道亲和性会基于从请求上下文或 JSON Body 提取的 Key，优先复用上一次成功的渠道。"} />;
  return <>
      <Spin spinning={loading}>
        <Form values={inputs} getFormApi={formAPI => refForm.current = formAPI} style={{
        marginBottom: 15
      }}>
          ...
          <Form.Section text={"渠道亲和性"}>
            ...
            <Row gutter={16}>
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Switch field={KEY_ENABLED} label={"启用"} checkedText='|' uncheckedText='O' onChange={value => setInputs({
                ...inputs,
                [KEY_ENABLED]: value
              })} />
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.InputNumber field={KEY_MAX_ENTRIES} label={"最大条目数"} min={0} placeholder={"例如 100000…"} extraText={<Text type='tertiary' size='small'>
                      {"内存缓存最大条目数。0 表示使用后端默认容量：100000。"}
                    </Text>} onChange={value => setInputs({
                ...inputs,
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.InputNumber field={KEY_DEFAULT_TTL} label={"默认 TTL（秒）"} min={0} placeholder={"例如 3600…"} extraText={<Text type='tertiary' size='small'>
            ...
            marginTop: 12
              ...
              <Col xs={24} sm={12} md={8} lg={8} xl={8}>
                <Form.Switch field={KEY_SWITCH_ON_SUCCESS} label={"成功后切换亲和"} checkedText='|' uncheckedText='O' onChange={value => setInputs({
                ...inputs,
                [KEY_SWITCH_ON_SUCCESS]: value
              })} />
            ...
            marginBottom: 10
              ...
              <Button icon={<IconRefresh />} loading={cacheLoading} onClick={refreshCacheStats}>
            ...
            {editMode === 'visual' ? <Table columns={ruleColumns} dataSource={rules} rowKey='id' pagination={false} size='small' /> : <Form.TextArea field={KEY_RULES} label={"规则 JSON"} extraText={"规则为 JSON 数组；可视化与 JSON 模式共用同一份数据。"} placeholder={RULES_JSON_PLACEHOLDER} style={{
            width: '100%'
          }} autosize={{
            minRows: 10,
            maxRows: 28
      ...
      <Modal title={isEdit ? "编辑规则" : "新增规则"} visible={modalVisible} onCancel={() => {
      setModalVisible(false);
      setEditingRule(null);
      setModalInitValues(null);
      setModalAdvancedActiveKey([]);
      setParamTemplateDraft('');
      setParamTemplateEditorVisible(false);
    }} onOk={handleModalSave} okText={"保存"} cancelText={"取消"} width={720}>

        <Form key={`channel-affinity-rule-form-${modalFormKey}`} initValues={modalInitValues || {}} getFormApi={formAPI => {
        modalFormRef.current = formAPI;
      }}>

          <Form.Input field='name' label={"名称"} extraText={"规则名称（可读性更好，也会出现在管理侧日志中）。"} placeholder={"例如 prefer-by-conversation-id…"} rules={[{
          required: true
        }]} onChange={value => setEditingRule(prev => ({
          ...(prev || {}),
          name: value
        }))} />
          ...
          <Row gutter={16}>
            <Col xs={24} sm={12}>
              <Form.TextArea field='model_regex_text' label={"模型正则（每行一个）"} extraText={"必填。对请求的 model 名称进行匹配，任意一条匹配即命中该规则。"} placeholder={'^gpt-4o.*$\n^claude-3.*$…'} autosize={{
              minRows: 4,
              maxRows: 10
            }} rules={[{
              required: true
            ...
            <Col xs={24} sm={12}>
              <Form.TextArea field='path_regex_text' label={"路径正则（每行一个）"} extraText={"可选。对请求路径进行匹配；不填表示匹配所有路径。"} placeholder={'/v1/chat/completions\n/v1/responses…'} autosize={{
              minRows: 4,
              maxRows: 10
            }} />
          ...
          marginTop: 12
            ...
            <Col xs={24} sm={12}>
              <Form.Switch field='skip_retry_on_failure' label={"失败后不重试"} />
          ...
          <Collapse keepDOM activeKey={modalAdvancedActiveKey} onChange={activeKey => {
          ...
          setModalAdvancedActiveKey(keys.filter(Boolean));
            ...
            <Collapse.Panel header={"高级设置"} itemKey='advanced'>
              <Row gutter={16}>
                <Col xs={24}>
                  <Form.TextArea field='user_agent_include_text' label={"User-Agent include（每行一个，可不写）"} extraText={<Text type='tertiary' size='small'>
              ...
              <Row gutter={16}>
                <Col xs={24} sm={12}>
                  <Form.Input field='value_regex' label={"Value 正则"} placeholder='^[-0-9A-Za-z._:]{1,128}$' extraText={"可选。对提取到的亲和 Key 做正则校验；不填表示不校验。"} />

                </Col>
                <Col xs={24} sm={12}>
                  <Form.InputNumber field='ttl_seconds' label={"TTL（秒，0 表示默认）"} placeholder={"例如 600…"} min={0} extraText={<Text type='tertiary' size='small'>
                ...
                <Col xs={24}>
                  ...
                  border: '1px solid var(--semi-color-border)'
                    ...
                    flexWrap: 'wrap'
                      ...
                      <Space>
                        <Button size='small' type='primary' icon={<IconCode />} onClick={() => setParamTemplateEditorVisible(true)}>
                        ...
                        <Button size='small' type='tertiary' onClick={() => updateParamTemplateDraft('')}>
              ...
              <Row gutter={16}>
                <Col xs={24} sm={12}>
                  <Form.Switch field='include_using_group' label={"作用域：包含分组"} />
                ...
                <Col xs={24} sm={12}>
                  <Form.Switch field='include_rule_name' label={"作用域：包含规则名称"} />
          ...
          marginBottom: 8
            ...
            marginTop: 6
              ...
              {(CONTEXT_KEY_PRESETS || []).map(x => <Tag key={x.key} style={{
              marginRight: 6,
              marginBottom: 6
            }}>
                  {x.label}
          ...
          <Table columns={[{
          title: "类型",
          render: (_, __, idx) => <Select style={{
            width: 160
          }} optionList={KEY_SOURCE_TYPES} value={(editingRule?.key_sources?.[idx]?.type || 'gjson').trim()} aria-label={"Key 来源类型"} onChange={value => updateKeySource(idx, {
            type: value
          })} />
        }, {
          title: "Key 或 Path",
          render: (_, __, idx) => {
            ...
            return <Input placeholder={isGjson ? 'metadata.conversation_id' : 'user_id'} aria-label={"Key 或 Path"} value={isGjson ? src.path : src.key} onChange={value => updateKeySource(idx, isGjson ? {
              path: value
            } : {
              key: value
            })} />;
          ...
          title: "操作",
          width: 90,
          render: (_, __, idx) => <Button icon={<IconDelete />} theme='borderless' type='danger' title={"删除 Key 来源"} aria-label={"删除 Key 来源"} onClick={() => removeKeySource(idx)} />
      ...
      <ParamOverrideEditorModal visible={paramTemplateEditorVisible} value={paramTemplateDraft || ''} onSave={nextValue => {
