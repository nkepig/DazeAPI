...
export default function SettingsPaymentGateway(props) {
  ...
  useEffect(() => {
    if (props.options && formApiRef.current) {
      const currentInputs = {
        ...
        StripePromotionCodesEnabled: props.options.StripePromotionCodesEnabled !== undefined ? props.options.StripePromotionCodesEnabled : false
      };
  ...
  const submitStripeSetting = async () => {
    ...
    try {
      ...
      if (originInputs['StripePromotionCodesEnabled'] !== inputs.StripePromotionCodesEnabled && inputs.StripePromotionCodesEnabled !== undefined) {
        options.push({
      ...
      // 发送请求
      const requestQueue = options.map(opt => API.put('/api/option/', {
        key: opt.key,
        value: opt.value
      }));
  ...
  return <Spin spinning={loading}>
      <Form initValues={inputs} onValueChange={handleFormChange} getFormApi={api => formApiRef.current = api}>
        ...
        <Form.Section text={"Stripe 设置"}>
          ...
          <Banner type='info' description={`Webhook 填：${props.options.ServerAddress ? removeTrailingSlash(props.options.ServerAddress) : "网站地址"}/api/stripe/webhook`} />

          <Banner type='warning' description={`需要包含事件：checkout.session.completed 和 checkout.session.expired`} />
          ...
          xxl: 24
            ...
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Input field='StripeApiSecret' label={"API 密钥"} placeholder={"sk_xxx 或 rk_xxx 的 Stripe 密钥，敏感信息不显示"} type='password' />

            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Input field='StripeWebhookSecret' label={"Webhook 签名密钥"} placeholder={"whsec_xxx 的 Webhook 签名密钥，敏感信息不显示"} type='password' />

            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Input field='StripePriceId' label={"商品价格 ID"} placeholder={"price_xxx 的商品价格 ID，新建产品后可获得"} />
          ...
          marginTop: 16
            ...
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.InputNumber field='StripeUnitPrice' precision={2} label={"充值价格（x元/美金）"} placeholder={"例如：7，就是7元/美金"} />

            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.InputNumber field='StripeMinTopUp' label={"最低充值美元数量"} placeholder={"例如：2，就是最低充值2$"} />

            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.Switch field='StripePromotionCodesEnabled' size='default' checkedText='｜' uncheckedText='〇' label={"允许在 Stripe 支付中输入促销码"} />
